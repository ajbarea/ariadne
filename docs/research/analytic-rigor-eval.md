...
!!! abstract "At a glance"
    ...
    The brief's core challenge is **specification and validation**: proving the
    analytic product is right when there is no single correct answer. Ariadne
    answers it on three threads:

    - **Citation groundedness**: every claim is cited (recall) and entailed by its
      source (precision), checked by an NLI model.
    - **Tradecraft compliance**: ICD-203/206 estimative language and structured
      analysis, enforced by a lint.
    - **Eval harness**: planted-needle scoring plus an LLM rubric, so a weak model
      fails cleanly and a strong one can be certified.
