...
[charter](../ROADMAP.md#mission--charter):
...
Feasibility is rated against this machine + the current toolset; most P0/P1 are
already shipped, which is why they are P0/P1 and not aspirations.
...
| Per-product model + egress audit | C-gov (security) | profile name + egress class in `governance.json` + OTel | In design (ADR-0013) | 🟡 |
...
| User-selectable model profiles (governed) | Usability + C-gov | unknown profile rejected; air-gap omits cloud profiles | In design (ADR-0013) | 🟡 |
...
| Native multimodal embeddings | SC2 (media) | API-only = breaks air-gap + uncitable black box | Rejected (ADR-0008) |
...
- **The live edges are all P1/P2 governance + deployment** (ADR-0012 validation,
  ADR-0013 profiles) — exactly where current work sits.
